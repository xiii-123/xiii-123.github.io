...
description: 'Vim轻量级配置'
image: ''
tags: [vim]
category: 'vim'
draft: false 
---
默认的Vim没有代码补全等功能，用起来比较痛苦。一些配置引擎的方案又过于重量，繁琐。为了两全其美，找到了大佬的开源代码（膜拜）。
只需要配置一个简单的文件，即可实现轻量级的代码补全系统。
来自大佬韦易笑的文章：[Vim2021：超轻量级代码补全系统 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/349271041)
...
augroup END
```
使用时，可以将该代码保存为apc.vim文件，扔到`~/.vim/plugins`目录下，或者在`vimrc`的最后加上：
```bash
source /path/to/apc.vim
```
还有更简单粗暴的做法，直接将代码全部copy进`vimrc`文件中，反正也没多少行。